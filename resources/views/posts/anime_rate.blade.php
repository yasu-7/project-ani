    ...
    <x-slot name="slot">
        ...
        <div class="mx-auto max-w-screen-2xl px-4 md:px-8 p-4">
            <div class="rounded-lg bg-gray-50 px-4 py-6 md:py-8 lg:py-12">
                  ...
                  <h2 class="mb-4 text-center text-2xl font-bold text-gray-800 md:mb-6 lg:text-3xl">アニメを評価</h2>
            
                  <div class="mx-auto max-w-screen-md text-center text-gray-500 md:text-lg">
                    アニメの評価と感想を記入してください。</br>
                    感想は未記入でも可能です。
                  </div>
            </div>
        </div>
        ...
        <div class="mx-auto max-w-screen-2xl px-4 md:px-8 p-4 bg-gray-50">
            <div class="mx-auto flex max-w-screen-lg items-center gap-8">
                ...
                <div class="w-1/3 overflow-hidden rounded-lg border">
                  <div class="h-48 bg-gray-100">
                    <img src="{{$datas[$count-1]["images"]["recommended_url"]}}"  class="h-full w-full object-cover object-center" />
                  </div>
                </div>
                <!-- promo - end -->
                
                <div class="grid w-2/3 gap-8">
                    <div class="mb-1 font-bold text-2xl">{{$datas[$count-1]["title"]}}</div>
                </div>
            </div>
            ...
            <form action="/posts/{{$datas[$count-1]["id"]}}" method="POST">
                ...
                <div class="title">
                    <input type="hidden" name="post[title]" value="{{$datas[$count-1]["title"]}}"/>
                </div>
                
                <div class="flex flex-row justify-center pt-8">
                    <div class="flex justify-center w-2/3">
                        <div class="text-2xl font-bold">感想：</div>
                        <textarea name="post[body]" rows="3" placeholder="口コミや感想などを投稿" class="w-2/3"></textarea>
                    </div>
                ...
                <ul>
                    @foreach ($errors->all() as $error)
                        <li><div class='text-center font-semibold py-4'>{{ $error }}</div></li>
                    @endforeach
                </ul>
